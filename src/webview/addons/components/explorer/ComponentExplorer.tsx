...
import { InputStringWidget } from '../InputStringWidget';
...
export default function ComponentExplorer() {
  ...
  const handleMentionsChange = (event: React.ChangeEvent<HTMLInputElement>, newValue: string) => {
    setMentionsValue(newValue);
  };

  const renderUserSuggestion = (suggestion: any) => (
    <div>{suggestion.display}</div>
  );

  const renderTagSuggestion = (suggestion: any) => (
    <div>{suggestion.display}</div>
  );
